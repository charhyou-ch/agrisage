...
def enrich_produit(p):
    """Ajoute groupe_frac, groupe_irac, risque_abeilles à un dict produit."""
...
def get_produits(qs):
    ...
    Paramètres query string :
      culture    — filtre par culture homologuée (LIKE)
      usage      — type : fongicide|insecticide|herbicide|acaricide|nematicide|...
      ma         — recherche matière active (LIKE)
      statut     — homologue (défaut) | tous
      groupe_frac— filtre par groupe FRAC
      groupe_irac— filtre par groupe IRAC
      q          — recherche libre sur nom_commercial + matiere_active
    ...
    # Requête de base — on joint usages pour avoir les cultures
    sql_select = '''
...
def get_cultures(qs):
    """
    Paramètres :
      q    — recherche partielle sur nom_fr
      page — numéro de page
    """
    q = qs.get('q', [None])[0]
    try:
...
def generer_conseil_batch(items):
    """
    Accepte une liste de requêtes conseil et retourne une liste de résultats.
    Chaque item doit avoir : culture, ravageur, stade
    Optionnel : dar_max, historique_frac, historique_irac, nb_alternatives, ref
    """
    if not isinstance(items, list):
        raise ValueError('Le champ "items" doit être une liste.')
    if len(items) == 0:
        raise ValueError('La liste "items" est vide.')
    if len(items) > 20:
        raise ValueError('Maximum 20 items par requête batch.')
...
class AgriSageHandler(BaseHTTPRequestHandler):
    ...
    def _handle_conseil_batch(self, body, start):
        ...
        if not items:
            return self.send_json(400, {'error':{'code':'INVALID_PARAM',
                'message':'Champ requis : "items" (liste de requêtes conseil).'}})
        try:
